package com.jaky.myplayer.ui.vitamio;
...
public final class LibsChecker {
	...
	//			Intent i = new Intent();
	//			i.setClassName(Vitamio.getCompatiblePackage(), "com.jaky.myplayer.ui.activity.InitActivity");
	...
	public static final boolean checkVitamioLibs(Activity ctx, int msgID) {
		if ((!Vitamio.isInitialized(ctx)) && (!ctx.getIntent().getBooleanExtra("fromVitamioInitActivity", false))) {
			Intent i = new Intent();
			i.setClassName(ctx.getPackageName(), "com.jaky.myplayer.ui.activity.InitActivity");//io.vov.vitamio.activity.InitActivity
			i.putExtras(ctx.getIntent());
			i.setData(ctx.getIntent().getData());
